...
// El codigo modificado estara marcado con ---------- al final de la linea y un comentario de que se ha cmabiado
...
protected:
    ...
    struct TreeNode {
        ...
        int altura;   //altura
        int tam_i     //tamaño de hijo

        TreeNode(T const &e, Link i = nullptr, Link d = nullptr,
                 int alt = 1, int tam_i = 1) : elem(e), iz(i), dr(d), altura(alt) tam_i(tam_i) {} // Hemos metido el tam_1 --------------------
    };
...
protected:
    ...
    // Aqui lo que se cambia es que si se añade un nuevo hijo, si es menor que la raiz lo estamos añadiendo a la izquierda
    bool inserta(T const &e, Link &a) {
        ...
        if (a == nullptr) { // se inserta el nuevo elemento e
            ...
            ++nelems;
            crece = true;
        } else if (menor(e, a->elem)) { // Aqui es donde se inserta a la izquierda
            crece = inserta(e, a->iz);
            ++(a->tam_i); // -------------------------- Tiene un elemento más a la izquierda por lo tanto tenemos que incrementar i.
