...
class PriorityQueue
{
  ...
  std::vector<T> array; // El primer elemento está en la posición 1
...
public:
  ...
  PriorityQueue(std::vector<T> const &v_ini, Comparator c = Comparator()) : array(v_ini.size() + 1), antes(c)
  {
  ...
  void pop()
  {
    ...
    {
      array[1] = std::move(array.back()); // move permite ser más eficientes al cambiar valores
      array.pop_back();
      if (!empty())
        hundir(1);
    }
  ...
  /* Escritura del montículo. Método para depurar */
  void print()
  {
    std::cout << "[";
  ...
  void hundir(int i)
  {
    ...
    while (hijo <= size())
    {
      ...
      if (hijo < size() && antes(array[hijo + 1], array[hijo]))
        ++hijo;
      // flotar el hijo izquierdo si va antes que el elemento hundiéndose
